...
3. Запустіть додаток однією командою:
   ```bash
   docker compose up -d
   ```
...
5. Для зупинки додатку:
   ```bash
   docker compose down
   ```

Дані зберігаються у Docker volume `nl2sql-data`, тому вони не будуть втрачені при перезапуску контейнера.
